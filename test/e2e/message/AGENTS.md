...
| `javascript_web_quickstart` | Prove the published localhost-BFF JavaScript quickstart completes bidirectional durable messaging, SENDACK/receive, disconnect, reconnect, and offline sync in Chromium against a real 256-Hash-Slot single-node cluster with Token auth enabled; HTTP readiness precedes browser CONNECT with BFF-issued credentials. History must contain the offline message and the UI must display it once regardless of live/sync arrival order. Failure evidence is bounded to three PNGs of at most 2 MiB each. | `WK_E2E_DOCS_JAVASCRIPT_WEB=1 GOWORK=off go test -tags=e2e ./test/e2e/message/javascript_web_quickstart -count=1 -timeout 10m -p=1 -v` |
...
| `conversation_directory_multi_node` | Prove Channel-Leader-grouped hydration, live committed-head reads, unavailable-Leader isolation with cursor progress, `/conversation/retry` recovery, and UID membership reads through a four-node ingress outside the UID Slot replica set. | `GOWORK=off go test -tags=e2e ./test/e2e/message/conversation_directory_multi_node -count=1 -timeout 3m -p=1` |
...
| `webhook` | Prove single-node cluster post-commit callbacks and three-node `msg.before_send` admission, custom codes, mutation, timeout/error policies, and committed history through WKProto and HTTP. | `GOWORK=off go test -tags=e2e ./test/e2e/message/webhook -count=1 -timeout 2m -p=1` |
