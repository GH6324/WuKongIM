...
| `cluster` | `test/e2e/cluster/manager_browser_smoke` | Prove the production Manager bundle supports authenticated desktop routes, localized copy, not-found handling, and mobile navigation in Chromium against a real three-node cluster without failed responses or browser errors. | `WK_E2E_MANAGER_BROWSER=1 GOWORK=off go test -tags=e2e ./test/e2e/cluster/manager_browser_smoke -count=1 -timeout 5m -p=1 -v` |
