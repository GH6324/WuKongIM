...
| `migration` | `test/e2e/migration/original_v2` | Prove original v2 archive migration, native startup, credentials, full history, event/CMD/conversation semantics, new sequence/idempotency, empty groups, restart and Leader failover across 1→1, 1→3, 3→1, 3→3 and 3→5 cluster topologies. | `GOWORK=off go test -tags=e2e ./test/e2e/migration/original_v2 -count=1 -timeout=5m` |
