...
| `message` | `test/e2e/message/javascript_web_quickstart` | Prove the published localhost-BFF JavaScript quickstart uses BFF-issued credentials with Token auth enabled and completes bidirectional durable messaging, SENDACK/receive, disconnect, reconnect, and offline sync in Chromium against a real 256-Hash-Slot single-node cluster. History must contain the offline message after bounded BFF projection recovery and the UI must display it once regardless of live/sync arrival order. Failure summaries include only capture bounds and fixed-spec line numbers. Failure evidence is bounded to three PNGs of at most 2 MiB each. | `WK_E2E_DOCS_JAVASCRIPT_WEB=1 GOWORK=off go test -tags=e2e ./test/e2e/message/javascript_web_quickstart -count=1 -timeout 10m -p=1 -v` |
| `message` | `test/e2e/message/easy_sdk_jsonrpc` | Prove source-aligned EasySDK iOS v1.1.0 binary/camelCase and Android v1.0.4 text/snake_case JSON-RPC profiles complete bidirectional messaging, acknowledgments, ping correlation, disconnect cleanup, and reconnect against a real 256-slot single-node cluster without claiming SDK artifact execution. | `GOWORK=off go test -tags=e2e ./test/e2e/message/easy_sdk_jsonrpc -count=1 -timeout 2m -p=1` |
| `message` | `test/e2e/message/conversation_directory` | Prove membership-backed directory pagination, activation priority, monotonic badge state, hide/remove/rejoin transitions, and opaque cursor completion through public APIs. | `GOWORK=off go test -tags=e2e ./test/e2e/message/conversation_directory -count=1 -timeout 2m` |
| `message` | `test/e2e/message/conversation_directory_multi_node` | Prove a three-node membership directory batches hydration by Channel Leader, isolates an unavailable Leader as unresolved without blocking cursor progress, and resolves it through bounded retry after restart. | `GOWORK=off go test -tags=e2e ./test/e2e/message/conversation_directory_multi_node -count=1 -timeout 3m -p=1` |
...
| `cluster` | `test/e2e/cluster/goroutine_monitor` | Prove a real three-node cluster exposes current goroutine ownership by node and module through Manager realtime monitor, including selected-node filtering without Prometheus history. | `GOWORK=off go test -tags=e2e ./test/e2e/cluster/goroutine_monitor -count=1 -timeout 2m -p=1` |
| `message` | `test/e2e/message/bench_churn` | Prove immediate cross-node Bench Token creation/rotation and exact rejection, including a non-replica, plus identity-swap group churn with Gateway Token auth enabled. | `GOWORK=off go test -tags=e2e ./test/e2e/message/bench_churn -count=1 -timeout 2m` |
| `cluster` | `test/e2e/cluster/manager_browser_smoke` | Prove the production Manager bundle supports authenticated desktop routes, localized copy, not-found handling, and mobile navigation in Chromium against a real three-node cluster with Gateway Token auth enabled and all-node HTTP readiness, without failed responses or browser errors. | `WK_E2E_MANAGER_BROWSER=1 GOWORK=off go test -tags=e2e ./test/e2e/cluster/manager_browser_smoke -count=1 -timeout 5m -p=1 -v` |
...
| `migration` | `test/e2e/migration/original_v2` | Prove expiration-bearing histories and unapplied source logs fail before target creation across 1→1, 1→3, 3→1, 3→3 and 3→5; prove an original empty group retains credentials, membership and first-write behavior through native startup. | `GOWORK=off go test -tags=e2e ./test/e2e/migration/original_v2 -count=1 -timeout=5m` |
| `migration` | `test/e2e/migration/ordinary_history` | Prove original ordinary histories migrate across 1→1, 1→3, 3→1, 3→3 and 3→5 with retained credentials/fields, sequential appends, idempotency, full restart and Channel Leader failover. | `GOWORK=off go test -tags=e2e ./test/e2e/migration/ordinary_history -count=1 -timeout=12m -p=1` |
