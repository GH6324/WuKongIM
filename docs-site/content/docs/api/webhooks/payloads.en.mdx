...
The synchronous callback receives one uncommitted message and returns an allow, Payload replacement, or rejection decision. See [Before-send business callback](/en/guide/integration/webhooks#before-send-business-callback) for configuration and failure policies, and the OpenAPI reference below for all fields.
...
<OpenAPIPage document="wukongim-webhooks-beta-en" webhooks={[{"name":"msg.before_send","method":"post"},{"name":"msg.notify","method":"post"},{"name":"msg.offline","method":"post"},{"name":"user.onlinestatus","method":"post"}]} showDescription />
