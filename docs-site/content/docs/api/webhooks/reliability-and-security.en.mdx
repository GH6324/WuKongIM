...
For asynchronous notifications, only HTTP `200` is success. Every other status, connection error, or timeout fails the attempt. The response body is drained and discarded; it has no protocol meaning.
