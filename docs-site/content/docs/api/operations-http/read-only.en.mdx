...
`users/tokens` persists device tokens through the user use case and increments `accepted_users` only after successful writes. Without the user use case, its capability is false and requests return 501. Persistence failure returns 500; a durable prefix is not rolled back and can be upserted again with the same credentials. Lifecycle workers complete this preparation before CONNECT while Gateway Token authentication remains enabled.
