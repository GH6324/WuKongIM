...
This extended acceptance is still in development and has not passed the complete Linux CI matrix. The [acceptance run](https://github.com/WuKongIM/WuKongEasySDK-CSharp/actions/runs/34208420961) exposed server recovery failures around rejoin and the second fault. The configuration and boundaries below are draft coverage, not a complete three-node recovery guarantee for a released version.

The three-node acceptance pins public NuGet `WuKongEasySDK 1.0.0`, npm `easyjssdk 2.0.5`, and Node `24.3.0` native WebSocket, testing released C# and candidate source separately. The server is repair source `c3dc526de3bc3f91461f32618ebdee7997984058`, with harness `19d07e1a38307aea4f5d22c43bd48e19b83c5be4`; this source validation must not be attributed to released `v3.0.0-beta.9`.

Three loopback processes share 256 hash slots, ten logical Slots, three Slot voters, and three Channel replicas, with Token authentication and delivery enabled. The fixture first verifies bidirectional person messages between every ingress and C#/JS group fanout to both other members. It then kills C# ingress node 1 and JS ingress node 2 in turn. Clients must report disconnection and reject offline SEND. After actual Slot leaders recover, the Channel migration write fence clears, and every surviving ingress reports all three recipients online for one second (bounded by 45 seconds), the application replaces the endpoint and verifies communication while the original node remains stopped, then restarts it with its existing data and verifies again.

**The application owns address replacement.** Each SDK instance has one fixed endpoint and retries that endpoint. A trusted backend selects a surviving ingress and obtains its `/route`. Await disposal of the old instance before creating a replacement with the new URL and existing credentials, so old reconnect attempts have ended.

The pinned server retains the default 90-second presence route lease. Replacement login can temporarily return system error `15` because conflict handling still tries to notify the stopped owner. The fixture retries only this reproduced activation error for at most 110 seconds, disposing every failed instance and recording elapsed time; invalid credentials and other failures stop the test. This is bounded eventual recovery, not an immediate-switch guarantee or advice to retry every system error or SEND. An interrupted SEND can still have an unknown outcome. Slot authority changes can temporarily hide a still-connected recipient until heartbeat/touch recreates its route; an ACK obtained during that gap does not imply online delivery. The fixture waits for public authority presence before sending and records convergence time.

The fixture explicitly uses a 100 ms Channel migration scan with up to ten pages and four tasks per tick, covering every Slot in this scenario. The current scanner restarts at the first locally led Slot each tick, so its default one-page budget can starve later Slots; this test does not prove recovery for every channel under the default budget. All person channels are established before the fault, so creating new three-replica channels while a node is absent is outside the receipt.

[Three-node reproduction and report fields](https://github.com/WuKongIM/WuKongEasySDK-CSharp/blob/19d07e1a38307aea4f5d22c43bd48e19b83c5be4/docs/cluster-acceptance.md) explain how to build the exact server and run `python3 scripts/interop.py --transport native --topology three-node`, adding `--candidate` for C# source. Two cluster jobs join the existing four single-node jobs to produce eight reports, retaining package integrity, source identities, ACK/RECV evidence, connection errors, and owned-process cleanup. This covers same-host three-node WS, not multi-host partitions, multi-node WSS, large-group capacity, offline synchronization, or long-duration stability.
