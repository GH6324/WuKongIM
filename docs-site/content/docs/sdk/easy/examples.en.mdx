...
C++ project `0.1.0` is consumed as source through CMake, separately from the four-platform historical released-package acceptance above. Each terminal supplies its own backend-issued Token through `WKIM_TOKEN`.
