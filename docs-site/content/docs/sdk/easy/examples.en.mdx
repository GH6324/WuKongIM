...
The Rust repository is [WuKongIM/WuKongEasySDK-Rust](https://github.com/WuKongIM/WuKongEasySDK-Rust), source package version `0.1.0`, not yet published to crates.io. Follow the [Rust quickstart](/en/sdk/easy/rust/getting-started), pin revision `f6ce7dc68277b53c5268a314bc02120c5c2eb1d9`, and run `cargo run --locked --example chat` or `cargo run --locked --example roundtrip`. Both Rust identities use PC `2` for `device_flag`.

This source ran on macOS / Rust 1.86.0 against WuKongIM `132e46209d98fa0425cc0f88e7a97080cdad044d`, a 256 Hash Slot single-node cluster with Token authentication enabled. Rust/Rust and Rust/`easyjssdk@2.0.4` passed bidirectional Unicode messaging, heartbeat, manual reconnect and cleanup; an incorrect token was rejected. The JS peer used WEB `1`. This Rust source run is separate from the four released-package receipts above and does not cover WSS, capacity or long-term stability.
