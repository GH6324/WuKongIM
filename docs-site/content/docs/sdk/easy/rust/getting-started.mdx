...
该 example 创建两个 Rust 客户端，核对 Unicode Payload 与发送结果，跨多个心跳周期检查连接，并手动重连后重复收发。JS 互通步骤见仓库 [README](https://github.com/WuKongIM/WuKongEasySDK-Rust/blob/f6ce7dc68277b53c5268a314bc02120c5c2eb1d9/README.md#development-and-live-verification)。
...
2026-09-08，Rust 源码 `f6ce7dc68277b53c5268a314bc02120c5c2eb1d9` 在 macOS / Rust 1.86.0 上通过协议与生命周期测试、Clippy、示例构建和打包校验；连接 WuKongIM `132e46209d98fa0425cc0f88e7a97080cdad044d` 的 256 Hash Slot 单节点集群，在 `token_auth_on=true` 下通过 Rust/Rust 及 Rust/`easyjssdk@2.0.4` 在线双向 Unicode 收发、心跳、手动重连与清理，并验证错误 Token 被拒绝。

这是精确源码的运行证据，不是 crates.io 发布包验收；WSS 代理、物理设备、离线恢复、容量和长期稳定性不在这次验证范围内。返回 [WuKongEasySDK](/zh/sdk/easy) 查看其他平台。
