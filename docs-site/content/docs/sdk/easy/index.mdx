---
title: WuKongEasySDK
description: 选择 iOS、Android、Flutter、Web、C#、C++、Rust 或 Python 快速接入，使用固定版本或源码完成 Alice 与 Bob 的在线双向消息闭环。
---
...
<Cards>
  ...
  <Card title="Rust 快速接入" description="固定 Git 源码，使用 Tokio 管理收发、有限重连与清理。" href="/zh/sdk/easy/rust/getting-started" />
  <Card title="C# 快速接入" description="从固定源码接入 .NET 8，使用异步 API、独立实例和 await using 完成收发。" href="/zh/sdk/easy/csharp/getting-started" />
  <Card title="C++ 快速接入" description="vcpkg 自动安装依赖，以 C++17 和 CMake 完成连接、消息和线程清理。" href="/zh/sdk/easy/cpp/getting-started" />
  <Card title="Python 快速接入" description="固定源码 + asyncio，完成在线收发、心跳、重连和资源清理。" href="/zh/sdk/easy/python/getting-started" />
</Cards>
...
C++ 使用独立 `WKIM` 实例和 I/O 线程，保存 `ListenerId`，退出时移除监听并等待 `disconnect()` / `destroy()` 完成。回调中不能等待 SDK future，UI 和耗时工作应交给应用执行器。详见 [C++ 快速接入](/zh/sdk/easy/cpp/getting-started)。
...
[C++ 仓库](https://github.com/WuKongIM/WuKongEasySDK-CPP) 工程版本 `0.1.0`，本文固定源码 `3e367a908f42385ab9306f9708b7456399cace7d`，可通过本仓库维护的 vcpkg Git registry 自动安装 SDK 及依赖，或使用 CMake 源码安装；尚无预编译 SDK 压缩包，也未宣称被微软默认目录收录。它参考 JS `v2.0.4`，已在 WuKongIM `132e46209d98fa0425cc0f88e7a97080cdad044d` 上完成 C++/C++ 和 C++/JS 在线双向消息、重连、错误 Token 拒绝和在线清理，并通过独立的 WS/WSS 协议测试。环境与完整验证范围见 [C++ 验证记录](https://github.com/WuKongIM/WuKongEasySDK-CPP/blob/3e367a908f42385ab9306f9708b7456399cace7d/docs/VALIDATION.md)。该源码证据独立于上面四端的历史正式包验收。
...
[WuKongEasySDK-Python](https://github.com/WuKongIM/WuKongEasySDK-Python) 工程版本 `0.1.0`，固定源码 `c8cf8b0e66b2b684fc30eecd0f2aac8c0e25c04a`，使用 Python 3.11+ 与 asyncio。当前通过源码安装，尚未发布到 PyPI。已在 WuKongIM `0348c0539bbee420a859439695acdac911afa854`、开启 Token 鉴权的 256 Hash Slot 单节点集群上完成 Python/Python 与真实 JS `2.0.4` 双向消息、心跳、重新连接、错误 Token 拒绝和在线清理。详细步骤与验证范围见 [Python 快速接入](/zh/sdk/easy/python/getting-started)。该源码证据独立于四端历史正式包验收。
