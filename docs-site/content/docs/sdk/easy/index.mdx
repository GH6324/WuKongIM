---
title: WuKongEasySDK
description: 选择 iOS、Android、Flutter、Web、C# 或 Rust 快速接入，使用固定版本完成 Alice 与 Bob 的在线双向消息闭环。
---
...
Rust 使用每个身份一个 `Client`，clone 共享连接；`subscribe()` 返回有界接收器，`disconnect().await` 可再次连接，`destroy().await` 永久关闭。默认设备类别为 PC `2`。安装和完整生命周期见 [Rust 快速接入](/zh/sdk/easy/rust/getting-started)。
