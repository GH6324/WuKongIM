...
[群聊正式包验证记录](https://github.com/WuKongIM/WuKongEasySDK-Python/blob/feaa52e7f788d67fd596be6149ed5f5b84ae1f7a/docs/GROUP_VALIDATION.md)独立记录三节点 WSS、4 个 Python/JS 客户端、2 个群的 13 个阶段：成员投递、群隔离、加入/移除/重新加入、非成员与黑名单拒绝，以及四个客户端重连后的成员与权限保持。Python 3.11 和 3.14 的 PyPI 消费环境还实际运行群聊 CLI。每个阶段对不应收到消息的客户端观察 1 秒；这是功能验收，不是大群容量或恰好一次投递保证。独立 CI 的候选 wheel 记录与 PyPI 下载记录分开保留。

一次早期 Linux CI 的 JS 成员漏收仍在[问题 #7](https://github.com/WuKongIM/WuKongEasySDK-Python/issues/7)中待定位。验证记录保留该失败；后续诊断运行通过不代表这一偶发现象已经修复。
