...
想先看到实际收发效果，可按[运行官方示例](/zh/sdk/easy/examples#python)准备两端；下文说明如何接入自己的应用。
...
群聊使用 `WKIMChannelType.GROUP`，无需客户端订阅。由可信后端建立群、添加成员并管理权限；接收继续使用 `WKIMEvent.MESSAGE`。跨节点成员变更需要包含[成员缓存修复](https://github.com/WuKongIM/WuKongIM/commit/2a295e0d9881ef5356728a85d56b052c4b0d9c86)的服务端，只升级 Python 包无法修复旧服务端行为。Slot Leader 切换时在线路由需要恢复，不能保证切换期间连续投递。完整示例与版本条件见[群聊说明](https://github.com/WuKongIM/WuKongEasySDK-Python/blob/527f37c876326e7ad3cc48c89828c4c3ffed09fc/examples/group_chat.py)。

## 下一步

继续阅读[消息收发](/zh/guide/integration/messaging)与[上线检查](/zh/guide/integration/acceptance)。需要离线恢复、会话、未读或推送时，先查看 [SDK 选择](/zh/sdk)。[版本与验证记录](https://github.com/WuKongIM/WuKongIM/blob/main/docs/superpowers/reports/2026-09-08-easysdk-validation-history.md#python)保留各次验证的完整环境和范围。
