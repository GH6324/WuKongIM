...
flowchart TD
  ...
  Usecase -->|权限 / system UID / plugin BeforeSend| Router["4. channelappend Router"]
  Router -->|解析精确 Channel authority| Local{"当前节点是 authority？"}
  Local -->|否| RPC["Channel Append RPC"] --> Writer
  Local -->|是| Writer["5. authority-local single writer"]
  Writer -->|校验、幂等、message ID、有界准入| Cluster["6. pkg/cluster Channel service"]
  Cluster -->|应用最新元数据 / 转发到 Channel Leader| Commit["7. Leader durable append + ISR replication + HW commit"]
  Commit --> Ack["8. SENDACK success"]
  Commit -.-> Effects["9. 提交后副作用"]
  Effects --> Subscribers["订阅者分页 + presence 查询"]
  Effects --> Delivery["owner-node 在线投递"]
