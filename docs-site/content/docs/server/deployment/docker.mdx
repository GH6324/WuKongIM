...
默认抓取容器内部的 `127.0.0.1:5001/metrics`，Prometheus 监听 `127.0.0.1:9099`，Manager 自动使用该地址查询指标。数据保存在 `/var/lib/wukongim/prometheus`，随现有 `wukongim-data` Volume 持久化；默认保留 15 天，可通过 `prometheus.retention_time` 和 `prometheus.retention_size` 调整。
