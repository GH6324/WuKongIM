...
description: Use wkcli migrate to build a new v3 cluster from unmodified, stopped v2 backups, verify the data, and prepare client cutover and rollback.
---
...
On macOS, use `shasum -a 256` instead of `sha256sum`. Build for the actual runtime platform. For a delivery package, compare its archive digest with an independent delivery record before checking the enclosed files. Releases containing the unified CLI include `wkcli` beside `wukongim` in official archives and native packages. Check `wkcli version --output json` against the server identity.
