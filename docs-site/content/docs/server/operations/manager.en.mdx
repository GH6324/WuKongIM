...
- Enable `manager.auth_on` in production and replace the default password and `manager.jwt_secret`.
- Bind Manager to loopback or a management-network address; do not expose node port `5301` publicly.
- For individual remote access, use the [SSH tunnel in Linux deployment](/en/server/deployment/linux). For shared access, use an HTTPS proxy through a management network or VPN.
...
<Callout type="warn" title="Keep authentication and transport protection enabled">
