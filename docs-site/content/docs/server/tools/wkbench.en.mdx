...
Run these commands from the repository root with the Go version required by `go.mod`. If you have an installed binary, replace `go run ./cmd/wkbench` with `wkbench`. Copy the configurations first:
