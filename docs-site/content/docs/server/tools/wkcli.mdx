---
title: wkcli
description: 使用一个运维工具完成在线操作、压测、离线数据库传输与 v2 到 v3 迁移。
---

`wkcli` 是 WuKongIM 的统一运维工具。在线操作通过 HTTP 和 WKProto 入口执行，`db` 和 `migrate` 处理离线数据。包含统一 CLI 的官方二进制归档和原生安装包会同时提供同版本的 `wkcli` 与 `wukongim`，可用 `wkcli version --output json` 核对构建身份。
