...
<Callout type="warn" title="HTTP examples belong behind a trusted service boundary">
  The default Beta Gateway validates stored `/user/token` device credentials, but current Product HTTP routes have no general product authentication. Browsers, mobile clients, and device firmware must not hold these server capabilities directly; token writes and other HTTP mutations belong behind a trusted service boundary.
</Callout>
