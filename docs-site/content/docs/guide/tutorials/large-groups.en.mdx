...
<Callout type="warn" title="A single request can partially complete">
...
```mermaid
flowchart TD
  Snapshot["Product membership snapshot / version"] --> Batch["Read the next bounded UID batch"]
  Batch --> Update["POST subscriber_add or subscriber_remove"]
  Update --> Checkpoint["Persist checkpoint and result"]
  Checkpoint --> Reconcile{"Desired state equals observed state?"}
  Reconcile -->|no| Batch
  Reconcile -->|yes| Done["Synchronization complete"]
```
