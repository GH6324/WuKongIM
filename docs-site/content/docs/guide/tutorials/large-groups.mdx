...
<Callout type="warn" title="单次请求也可能部分完成">
...
```mermaid
flowchart TD
  Snapshot["业务成员快照 / 版本"] --> Batch["读取下一批有界 UID"]
  Batch --> Update["POST subscriber_add 或 subscriber_remove"]
  Update --> Checkpoint["持久化检查点与结果"]
  Checkpoint --> Reconcile{"期望状态等于观测状态？"}
  Reconcile -->|否| Batch
  Reconcile -->|是| Done["同步完成"]
```
