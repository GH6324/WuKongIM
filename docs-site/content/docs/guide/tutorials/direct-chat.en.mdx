...
```mermaid
sequenceDiagram
  participant Alice as Alice product Session
  participant Gateway
  participant Channel as Canonical direct Channel
  participant Bob as Bob product Session
  Alice->>Gateway: SDK CONNECT
  Bob->>Gateway: SDK CONNECT
  Alice->>Gateway: SEND peer=bob, type=1
  Gateway->>Channel: canonicalize and append
  Channel-->>Alice: SENDACK after quorum commit
  alt Bob is online
    Channel-->>Bob: RECV
  else Bob is offline
    Bob->>Channel: synchronize after reconnect
    Channel-->>Bob: missing messages
  end
```
...
<Callout type="warn" title="The built-in check is not a complete production identity system">
