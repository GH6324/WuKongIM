...
| --- | --- | --- |
| Linux / Docker single-node cluster | `http://127.0.0.1:5001/demo/` | Same origin as the page |
...
If the page opens but WebSocket fails, check that Docker mappings or SSH forwarding include `5200`. Remapped ports, separate domains, and HTTPS/WSS proxies need explicit published addresses and a working TLS/WebSocket ingress; see [Docker Deployment](/en/server/deployment/docker).
