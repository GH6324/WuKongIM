...
Start with the [Quick Start](/en/guide/quick-start) to run a single-node cluster and send the first message. During integration, read [Messaging Integration](/en/guide/integration/messaging). Before launch, complete [Integration Acceptance](/en/guide/integration/acceptance), then use [`wkcli bench`](/en/server/tools/wkbench) to validate capacity with a representative workload.
