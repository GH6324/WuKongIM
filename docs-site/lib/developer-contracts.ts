...
export const messageHeaderFlags: MessageHeaderFlagDefinition[] = [
  messageHeaderFlag(0, 'NoPersist', '普通和命令式消息均进入瞬时在线投递；分配消息 ID，序号为零，不写入持久历史，也不能离线恢复。', 'Ordinary and command-style messages enter transient online delivery with a message ID and sequence zero. Neither writes durable history or supports offline recovery.'),
