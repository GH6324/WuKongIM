...
BASE_SCRIPT="${WK_BENCH_SINGLE_REAL_QPS_BASE_SCRIPT:-$ROOT_DIR/scripts/bench-wukongim-single-node-1000ch.sh}"
...
usage() {
  cat <<'USAGE'
Usage: scripts/bench-wukongim-single-node-real-qps.sh [options]

Runs a true QPS search shape for local cmd/wukongim single-node cluster group-channel QPS runs.
...
Example:
  GOWORK=off scripts/bench-wukongim-single-node-real-qps.sh --qps 2400,2600,2800
...
## Scenario
- workload: clean single-attempt local wukongim single-node cluster group-channel QPS runs
- qps_list: $QPS_LIST
- channels: $CHANNELS
- users: $USERS
- group_members: $GROUP_MEMBERS
