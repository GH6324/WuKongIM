...
ssh_load "sudo -u wukongim /opt/wukongim/bin/wkcli bench validate chat-lifecycle --config /etc/wukongim/chat-lifecycle.yaml >/dev/null && sudo -u wukongim /opt/wukongim/bin/wkcli bench validate chat-lifecycle --config /etc/wukongim/chat-lifecycle-rehearsal.yaml >/dev/null" && workload_valid=true
