...
func testStoreContract(t *testing.T, factory Factory) {
	...
	appendRes, err := cs.AppendLeader(ctx, AppendLeaderRequest{Records: []ch.Record{{ID: 1, Payload: []byte("a"), SizeBytes: 1, SyncOnce: true, RedDot: true}, {ID: 2, Payload: []byte("b"), SizeBytes: 1}}})
	require.NoError(t, err)
	...
	require.True(t, logRes.Records[0].RedDot)
	require.False(t, logRes.Records[1].RedDot)
