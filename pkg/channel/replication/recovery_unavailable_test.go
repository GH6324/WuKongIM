...
func TestQuorumInstallPreservesAcknowledgedTailWithUnavailableVoter(t *testing.T) {
	for _, name := range []string{"empty-prefix", "nonempty-prefix"} {
		t.Run(name, func(t *testing.T) {
			...
			for _, voter := range []ch.NodeID{1, 3} {
			...
			if loadErr != nil || len(loaded.Items) != 1 || loaded.Items[0].Err != nil || loaded.Items[0].State.LEO != base+1 || loaded.Items[0].State.Manifest != mutation.Manifest {
			...
			if !errors.Is(err, errRecoveryProbeIncomplete) {
				t.Fatalf("Install = %v; want incomplete evidence", err)
			}
