...
// NewRedactionCommand validates and redacts product configuration for benchmark evidence.
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			if strings.TrimSpace(inputPath) == "" || strings.TrimSpace(outputPath) == "" {
				return command.Exit{Code: command.ExitConfig, Message: "--input and --output are required"}
			}
