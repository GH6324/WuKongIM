...
`wkcli bench` is a black-box benchmark driver for WuKongIM. It talks to a running WuKongIM cluster through public HTTP, benchmark-only HTTP, and WKProto gateway endpoints. It does not import server internals or bypass cluster semantics; a single-node deployment is treated as a single-node cluster.

## Commands

```bash
go run ./cmd/wkcli bench <command> [flags]
```
...
Enable the server-side bench API in `wukongim.toml` before running wkbench:
...
For a compiled binary, replace `go run ./cmd/wkcli bench` with the binary path followed by `bench` (for example, `/usr/bin/wkcli bench`).
...
and a bounded recent transition log. `wkcli bench` also emits one compact
...
```bash
wkcli bench report chat-lifecycle-timeline \
...
```bash
wkcli bench report chat-lifecycle-cut-query \
  --worker-log ./tmp/chat-lifecycle-shakeout/logs/coordinator.log \
  --run-id local-chat-lifecycle-shakeout \
  --cursor 0 \
  --offered-rate 1000 \
...
```bash
wkcli bench capacity hot-channel \
...
```bash
wkcli bench capacity activate-channels \
...
```bash
wkcli bench capacity message-event \
  --api http://127.0.0.1:5001 \
...
cluster, builds `wkcli bench` when needed, runs `capacity message-event`, and
...
```bash
wkcli bench dev-sim --config /etc/wkbench/dev-sim.yaml
```

For local non-Docker runs:

```bash
GOWORK=off go run ./cmd/wkcli bench dev-sim --config ./docker/sim/dev-sim.yaml
```
...
```bash
GOWORK=off go test ./internal/bench/... ./cmd/wkcli/internal/benchmark -count=1
GOWORK=off go test ./cmd/wkcli/internal/benchmark -run TestWkbenchDoesNotImportServerInternals -count=1
GOWORK=off go test -tags=e2e ./test/e2e/bench/wkbench_smoke -count=1
```

Keep `cmd/wkcli/internal/benchmark` as a thin CLI over `internal/bench`. Do not import WuKongIM server internals into wkbench; use `internal/bench/target` and WKProto clients as black-box boundaries.
