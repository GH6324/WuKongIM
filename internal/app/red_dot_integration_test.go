...
// TestRedDotHTTPCluster covers ordinary HTTP send/read through every entry
// node and a single-node cluster restart, using both flag values.
func TestRedDotHTTPCluster(t *testing.T) {
		...
		t.Run(name, func(t *testing.T) {
			...
			for i, a := range apps {
				for _, flag := range []int{0, 1} {
					client := fmt.Sprintf("red-dot-%d-%d", i, flag)
					raw := postAppJSON(t, a.api.(*accessapi.Server).Handler(), "/message/send",
						fmt.Sprintf(`{"header":{"red_dot":%d},"from_uid":"red-dot-sender","channel_id":"red-dot-receiver","channel_type":1,"client_msg_no":%q,"payload":"aGVsbG8="}`, flag, client), http.StatusOK)
					var sent sentMessage
					require.NoError(t, json.Unmarshal(raw, &sent))
					require.NotZero(t, sent.MessageID)
					require.NotZero(t, sent.MessageSeq)
					sent.ClientMsgNo, sent.Payload, sent.Header.RedDot = client, "aGVsbG8=", flag
				...
				for _, a := range apps {
					...
					for time.Now().Before(deadline) {
						...
						req := httptest.NewRequest(http.MethodPost, "/channel/messagesync", strings.NewReader(
							`{"login_uid":"red-dot-receiver","channel_id":"red-dot-sender","channel_type":1,"limit":100}`)).WithContext(ctx)
